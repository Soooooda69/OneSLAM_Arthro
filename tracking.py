...
class Tracking:
    ...
    def process_section(self, section_indices, dataset, slam_structure, 
        ...
        # Point resampling process
        if sample_new_points:
            # image_0 = self.dataset[frame_idx]['image'].permute(1, 2, 0).detach().cpu().numpy()*255
            ...
            # if section_indices[start_frame] not in slam_structure.all_frames.keys():
            #     pose = np.identity(4)
            #     frame = Frame(frame_idx, pose, mask_0, image_0, intrinsics_0)
            #     slam_structure.all_frames[section_indices[start_frame]] = frame
            # else:
            #     frame = slam_structure.all_frames[section_indices[start_frame]]
            #     # pose_0 = frame.itransform_matrix
