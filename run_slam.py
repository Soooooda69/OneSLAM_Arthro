...
parser.add_argument('--depth_scale',  default=10.0, type=float, help='scaling factor for depth esimates')
...
elif args.pose_guesser == 'gt_pose':
    pose_guesser =  PoseGuesserGTPose(args.gt_pose_path)
else:
    raise ValueError(f'Unknown argument for --pose_guesser: {args.pose_guesser}')


# Depth estimation component
if args.depth_estimator == 'constant':
    depth_estimator = DepthEstimatorConstant(args.depth_scale)
elif args.depth_estimator == 'luminosity':
    # TODO: Implement
    raise NotImplementedError("Not implemented yet")
elif args.depth_estimator == 'depth_network':
    # Path to current depth estimator: "./trained_models/depth/depth_estimator_8_sequences_lowest_val.pt"
    depth_estimator =  DepthEstimatorFCDenseNet57(args.depth_network_path, args.target_device, args.depth_scale)
else:
    raise ValueError(f'Unknown argument for --depth_estimator: {args.depth_estimator}')
...
elif args.point_sampler == 'akaze':
    point_sampler = PointResamplerAKAZE(args.tracked_point_num_min, args.tracked_point_num_max, args.minimum_new_points)
else:
