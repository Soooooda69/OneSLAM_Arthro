...
class CoTrackerPredictor(torch.nn.Module):
    ...
    def _compute_sparse_tracks(
        ...
        if add_support_grid:
            ...
            local_support_grid_size = self.support_grid_size**2
            if segm_mask is not None:
                segm_mask = F.interpolate(
                    segm_mask, tuple(self.interp_shape), mode="nearest"
                )
                ...
                grid_pts = grid_pts[:, point_mask]
                local_support_grid_size = grid_pts.shape[1]
        ...
        if backward_tracking:
            ...
            if add_support_grid:
                queries[:, -local_support_grid_size :, 0] = T - 1
        if add_support_grid:
            tracks = tracks[:, :, : -local_support_grid_size]
            visibilities = visibilities[:, :, : -local_support_grid_size]
        #print(local_support_grid_size)
        thr = 0.9
